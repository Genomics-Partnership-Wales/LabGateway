...
public class PoisonQueueRetryOrchestratorTests
{
    ...
    public async Task ProcessPoisonQueueAsync_ProcessesEachMessageByCallingProcessSingleMessageAsync()
    {
        ...
        _messageProcessorMock.Setup(x => x.ProcessMessageAsync(It.IsAny<QueueMessageDto>()))
        ...
        _messageProcessorMock.Verify(
            x => x.ProcessMessageAsync(It.Is<QueueMessageDto>(dto => dto.MessageId == "msg1")),
            Times.Once);
        _messageProcessorMock.Verify(
            x => x.ProcessMessageAsync(It.Is<QueueMessageDto>(dto => dto.MessageId == "msg2")),
            Times.Once);
        _messageProcessorMock.Verify(
            x => x.ProcessMessageAsync(It.Is<QueueMessageDto>(dto => dto.MessageId == "msg3")),
            Times.Once);
    }
    ...
    public async Task ProcessPoisonQueueAsync_DeletesMessage_WhenResultIsSuccess()
    {
        ...
        _azureQueueClientMock.Verify(x => x.UpdateMessageAsync(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<TimeSpan>()), Times.Never);
    ...
    public async Task ProcessPoisonQueueAsync_UpdatesMessage_WhenResultIsRetry()
    {
        ...
        // Assert
        _azureQueueClientMock.Verify(x => x.UpdateMessageAsync("msg1", "receipt1", TimeSpan.FromMinutes(4)), Times.Once);
