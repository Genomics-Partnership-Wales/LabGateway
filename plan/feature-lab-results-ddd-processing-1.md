...
2. BlobTrigger Azure Function (FileProcessor) activates
...
9. Timer-triggered function (TimeTriggeredProcessor) retries poison queue messages (max 3 attempts)
...
| Task | Description | Completed | Date |
|------|-------------|-----------|------|
| TASK-016 | Create `src/API/Infrastructure/` folder structure with subfolders: `ExternalServices/`, `Hl7/`, `Messaging/`, `Storage/` | | |
| TASK-017 | Add NuGet package `NHapi.Base` latest v3.x version supporting .NET 10.0 to `LabResultsGateway.API.csproj` | | |
| TASK-018 | Add NuGet package `NHapi.Model.V251` latest version to `LabResultsGateway.API.csproj` for HL7 v2.5.1 support | | |
| TASK-019 | Add NuGet packages for OpenTelemetry: `OpenTelemetry.Extensions.Hosting`, `OpenTelemetry.Instrumentation.Http`, `OpenTelemetry.Exporter.AzureMonitor` latest versions to `LabResultsGateway.API.csproj` | | |
| TASK-020 | Add NuGet package `Polly` v8.x (resilience framework) to `LabResultsGateway.API.csproj` | | |
| TASK-021 | Add NuGet package `Azure.Extensions.AspNetCore.Configuration.Secrets` for Key Vault integration to `LabResultsGateway.API.csproj` | | |
